import { Animation, AnimationCallbackOptions, AnimationDirection, AnimationFill, AnimationPlayOptions } from './../../api';
import { AnimationKeyFrame, AnimationKeyFrameEdge, AnimationKeyFrames } from './../../api';
import { AnimationLifecycle } from './../../api';
...
export const createAnimation = (animationId?: string): Animation => {
  ...
  const supportsWebAnimations =
    typeof (Element as any) === 'function' && typeof (Element as any).prototype!.animate === 'function' && supportsAnimationEffect;
  ...
  /**
   * Cancels any WebAnimations, removes any animation properties from the animation's elements
   * and removes the animation's stylesheets from the DOM.
   */
  const cleanUp = () => {
    cleanUpElements();
    cleanUpStylesheets();
  ...
  const onFinish = (cb: AnimationLifecycle, opts?: AnimationCallbackOptions) => {
    const callbacks = opts && opts.oneTimeCallback ? onFinishOneTimeCallbacks : onFinishCallbacks;
    callbacks.push({ c: cb, o: opts });
    return ani;
  };
  const clearOnFinish = () => {
  ...
  /**
   * Cancels any Web Animations and removes any animation properties from the animation's elements.
   */
    ...
    if (supportsWebAnimations) {
      ...
      raf(() => {
        elementsArray.forEach(el => {
          Util.removeStyleProperty(el, 'animation-name');
          Util.removeStyleProperty(el, 'animation-duration');
          Util.removeStyleProperty(el, 'animation-timing-function');
          Util.removeStyleProperty(el, 'animation-iteration-count');
          Util.removeStyleProperty(el, 'animation-delay');
          Util.removeStyleProperty(el, 'animation-play-state');
          Util.removeStyleProperty(el, 'animation-fill-mode');
          Util.removeStyleProperty(el, 'animation-direction');
        });
      });
    }
  };
  ...
  /**
   * Removes the animation's stylesheets from the DOM.
   */
  const cleanUpStylesheets = () => {
    stylesheets.forEach(stylesheet => {
      // when sharing stylesheets, it's possible for another animation to have already
  ...
  const beforeAddClass = (className: string | string[] | undefined) => {
    beforeAddClasses = Util.addClassToArray(beforeAddClasses, className);

    return ani;
  };

  const beforeRemoveClass = (className: string | string[] | undefined) => {
    beforeRemoveClasses = Util.addClassToArray(beforeRemoveClasses, className);
  ...
  const afterAddClass = (className: string | string[] | undefined) => {
    afterAddClasses = Util.addClassToArray(afterAddClasses, className);

    return ani;
  };

  const afterRemoveClass = (className: string | string[] | undefined) => {
    afterRemoveClasses = Util.addClassToArray(afterRemoveClasses, className);
    ...
    elements.forEach(el => {
      ...
      for (const property in styles) {
        if (styles.hasOwnProperty(property)) {
          Util.setStyleProperty(el, property, styles[property]);
        }
      }
    });
  };
  ...
  const initializeCSSAnimation = (toggleAnimationName = true) => {
    ...
    const processedKeyframes = Util.processKeyframes(_keyframes);
    elements.forEach(element => {
      if (processedKeyframes.length > 0) {
        const keyframeRules = Util.generateKeyframeRules(processedKeyframes);
        keyframeName = animationId !== undefined ? animationId : Util.generateKeyframeName(keyframeRules);
        const stylesheet = Util.createKeyframeStylesheet(keyframeName, keyframeRules, element);
        stylesheets.push(stylesheet);

        Util.setStyleProperty(element, 'animation-duration', `${getDuration()}ms`);
        Util.setStyleProperty(element, 'animation-timing-function', getEasing());
        Util.setStyleProperty(element, 'animation-delay', `${getDelay()}ms`);
        Util.setStyleProperty(element, 'animation-fill-mode', getFill());
        Util.setStyleProperty(element, 'animation-direction', getDirection());

        const iterationsCount = getIterations() === Infinity ? 'infinite' : getIterations().toString();

        Util.setStyleProperty(element, 'animation-iteration-count', iterationsCount);
        Util.setStyleProperty(element, 'animation-play-state', 'paused');

        if (toggleAnimationName) {
          Util.setStyleProperty(element, 'animation-name', `${stylesheet.id}-alt`);
        }

        raf(() => {
          Util.setStyleProperty(element, 'animation-name', stylesheet.id || null);
        });
      }
    });
  };
  ...
  const setAnimationStep = (step: number) => {
    ...
    if (supportsWebAnimations) {
      ...
      elements.forEach(element => {
        if (_keyframes.length > 0) {
          Util.setStyleProperty(element, 'animation-delay', animationDuration);
          Util.setStyleProperty(element, 'animation-play-state', 'paused');
        }
      });
    }
  ...
  const updateCSSAnimation = (toggleAnimationName = true, step?: number) => {
    raf(() => {
      elements.forEach(element => {
        Util.setStyleProperty(element, 'animation-name', keyframeName || null);
        Util.setStyleProperty(element, 'animation-duration', `${getDuration()}ms`);
        Util.setStyleProperty(element, 'animation-timing-function', getEasing());
        Util.setStyleProperty(element, 'animation-delay', step !== undefined ? `-${step! * getDuration()}ms` : `${getDelay()}ms`);
        Util.setStyleProperty(element, 'animation-fill-mode', getFill() || null);
        Util.setStyleProperty(element, 'animation-direction', getDirection() || null);
        ...
        if (toggleAnimationName) {
          Util.setStyleProperty(element, 'animation-name', `${keyframeName}-alt`);
        }
  ...
  const playCSSAnimations = () => {
    ...
    raf(() => {
      elements.forEach(element => {
        if (_keyframes.length > 0) {
          Util.setStyleProperty(element, 'animation-play-state', 'running');
        }
      });
    });
  ...
  const clearCSSAnimationPlayState = () => {
    elements.forEach(element => {
      Util.removeStyleProperty(element, 'animation-duration');
      Util.removeStyleProperty(element, 'animation-delay');
      Util.removeStyleProperty(element, 'animation-play-state');
    });
  };
